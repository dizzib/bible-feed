...
class SyncOut extends StatelessWidget {
  ...
  build(context) {
    final Encode result = zx.encodeBarcode(
      ...
      params: EncodeParams(format: Format.qrCode, width: 120, height: 120, margin: 10, eccLevel: EccLevel.low),
    );

    if (!result.isValid || result.data == null) return const Text('Could not generate QR code');

    final img = imglib.Image.fromBytes(width: 120, height: 120, bytes: result.data!.buffer, numChannels: 1);
